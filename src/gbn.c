/**
 * Reference taken from the pseudo-code of the lecture slides
 */
#include <stdio.h>
#include <stdlib.h>
#include <getopt.h>
...
int base;// stores the value of the base of the window.Used by the sender
int nextseqnum;//stores the value of the next sequence number.Used by the sender
int expectedseqnum;//expected sequence number at the receiver side
struct pkt sndpkt[5000];//assumption - no. of packets will never be more than 5000
char msg[20];//var for storing the message
struct pkt ackpkt;//global ackpacket
...
/*
* Function which sets the message from app layer into the var char msg[20]
*/
...
//Pseudocode referred from the recitation slides

/* called from layer 5, passed the data to be sent to other side */
/* Reference taken from lecture slide*/
  ...
  struct msg message;
{
	...
	if(nextseqnum<base+WINSIZE){
    	...
    	strncpy(sndpkt[nextseqnum].payload,message.data,20);
    	int checksum = calculatechecksum(sndpkt[nextseqnum]);
    	sndpkt[nextseqnum].checksum = checksum;
...
void A_input(packet)
  struct pkt packet;
{
    //printf("[A_input]: nextseqnum %d",nextseqnum);
    //printf("[A_input]: ACK received for seq %d",packet.acknum);
	int checksum = calculatechecksum(packet);
    //if the packet is not corrupted
    if(checksum==packet.checksum){
       base = packet.acknum + 1;
       //printf("\n[A_input]: Base updated to %d",base);
       if(base == nextseqnum){
    	   stoptimer(0);
       }
       else{
	   ...
	   while(send_buffer_start!=send_buffer_end && nextseqnum<base+WINSIZE){
		   //printf("\nInside buffer");
		    //make packet
			sndpkt[nextseqnum].seqnum = nextseqnum;
			strncpy(sndpkt[nextseqnum].payload,send_buffer[send_buffer_start].data,20);
			int checksum = calculatechecksum(sndpkt[nextseqnum]);
			sndpkt[nextseqnum].checksum = checksum;
...
void A_timerinterrupt()
{
	 ...
	 starttimer(0,TIMEOUT);
     int i;
     //printf("\n[A_timerinterrupt]: Retransmitting packets %d to %d",base,nextseqnum-1);
     //retransmit all the packets from base in the window
     for(i=base;i<nextseqnum;i++){
    	 tolayer3(0,sndpkt[i]);
    	 A_transport++;
    	 //printf("\n[A_timerinterrupt]: Retransmitting packet %d",i);
     }
}  
  ...
  struct pkt packet;
{
	...
	int checksum = calculatechecksum(packet);
/*    printf("\n=============================================");
    printf("\n[B_input]: Expected seqnum %d",expectedseqnum);
	printf("\n[B_input]: Packet seqnum %d",packet.seqnum);
	printf("\n[B_input]: Checksum %d",checksum);
	printf("\n[B_input]: Packet checksum %d",packet.checksum);
	printf("\n=============================================");*/
	//if the packet is not corrupt and it is expected
	if(checksum == packet.checksum && expectedseqnum==packet.seqnum){
		   //Deliver packet to the application
		   ...
		   tolayer3(1,ackpkt);
		   //increase the next expected sequence num
		   expectedseqnum++;
	}
}
