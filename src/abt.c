...
int A_seqnum; // maintains the seq no. to be sent over to B
int intransit;// flag to check if there is an intransit message.
struct pkt sndpkt; // global var that holds the value of the packet in transit
char msg[20];//var for storing the message
int B_seqnum; // maintains the seq no. to be sent over to B
struct pkt ackpacket; // ack packet
  ...
  struct msg message;
{
  ...
  /**
   * make the packet to be sent to B if there is no intransit message
   * else the message is discarded
   */
   if(!intransit){
	   //make packet
	   sndpkt.seqnum = A_seqnum;//alternate the sequence no. between 0 and 1
	   setmessage(message.data);
	   strncpy(sndpkt.payload,msg, 20);// set the payload
	   sndpkt.checksum = calculatechecksum(sndpkt);//find the checksum
	   //printf("\nA_output SEQ:: %d",sndpkt.seqnum);
  ...
  struct pkt packet;
{
	...
	//printf("\nA_input ACK:: %d",packet.acknum);
	if((checksum==packet.checksum) && (A_seqnum==packet.acknum)){
			A_seqnum=(A_seqnum+1)%2;//alternate the sequence
			//printf("\nA_input Stopping timer A_SEQNUM:: %d",A_seqnum);
            //stop the timer
			stoptimer(0);
			intransit=0;
	}
...
/*		printf("\n=================================================");
		printf("\nA_input Check failed");
		printf("\nChecksum %d",checksum);
		printf("\nPacket checksum %d",packet.checksum);
		printf("\nA_seqnum  %d",A_seqnum);
		printf("\nPacket acknum %d",packet.acknum );
		printf("\n=================================================");*/
	}
}
...
void B_input(packet)
  struct pkt packet;
{
	printf("\nB_input SEQ:: %d",packet.seqnum);
    B_transport++;
	int checksum = calculatechecksum(packet);
	if(checksum==packet.checksum && packet.seqnum==B_seqnum){
		//printf("\nB_input Message delivered to app layer %s",packet.payload);
        ...
        checksum = calculatechecksum(ackpacket);
        ackpacket.checksum = checksum;
        tolayer3(1,ackpacket);
	}else{
		//send ACK of the previous packet
        tolayer3(1,ackpacket);
	}
}
...
{
	B_seqnum = 0;
	//make a ACK for the first message from A. Handled the scenario if the first packet is corrupted itself.
	ackpacket.acknum = 1;
	int checksum = calculatechecksum(ackpacket);
	ackpacket.checksum = checksum;
}
